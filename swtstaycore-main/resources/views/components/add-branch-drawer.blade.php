...
<div x-data="{ show: {{ $autoShow ? 'true' : 'false' }}}"
    x-show="show"
    x-init="if ({{ $autoShow ? 'true' : 'false' }}) { document.body.classList.add('overflow-hidden') }"
    x-on:open-add-branch-drawer.window="show = true;  document.body.classList.add('overflow-hidden')"
    x-on:close-add-branch-drawer.window="show = false; document.body.classList.remove('overflow-hidden')"
        ...
        <div class="overflow-y-auto shadow-lg bg-white rounded h-full">
            ...
            <div class="sticky top-0 flex bg-white z-10 justify-between items-center rounded-t-3xl border-b sm:mb-5 px-5 py-5">
                ...
                @if (Auth::user()->hotelOwner->is_branch_added)
                    <button type="button" x-on:click="$dispatch('close-add-branch-drawer')"
