...
<div x-data = "{ show : false }" x-show = "show"
    x-on:open-add-team-modal.window = "show = true; document.body.classList.add('overflow-hidden')"
    x-on:close-add-team-modal.window = "show = false; document.body.classList.remove('overflow-hidden')"
    ...
    {{-- modal content --}}
    <div class="shadow-md bg-gray-100/50 backdrop-blur-sm rounded-3xl border border-primary-50 m-auto p-1 fixed inset-0 max-w-xl max-h-96" style="max-height: 650px">
        <div class="overflow-y-auto shadow-lg bg-white rounded-3xl border border-primary-100  scrollbar-hide hover:scrollbar-show h-full">
            ...
            <div class="sticky top-0 flex bg-white z-10 justify-between items-center rounded-t-3xl border-b sm:mb-5 px-8 py-5">
                <div class="">
                    <h3 class="text-lg font-semibold text-gray-900">Add Hotel Staff</h3>
                    {{-- <p class="text-gray-400 font-light">Please provide the following information</p> --}}
                </div>
                {{-- @if (Auth::user()->hotelOwner->is_team_added) --}}
                    <button type="button" x-on:click="$dispatch('close-add-team-modal')"
