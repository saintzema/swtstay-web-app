...
<div x-data="{ show: false }"
    ...
    x-on:open-edit-hotel-drawer.window="show = true;  document.body.classList.add('overflow-hidden')"
    x-on:close-edit-hotel-drawer.window="show = false; document.body.classList.remove('overflow-hidden')"
    ...
    <div class="shadow-md bg-gray-100/95 backdrop-blur-sm p-2 w-full max-w-lg h-full fixed right-0">
        <div class="overflow-y-auto shadow-lg bg-white rounded h-full">
            ...
            <div class="sticky top-0 flex bg-white z-10 justify-between items-center rounded-t-3xl border-b sm:mb-5 px-5 py-5">
                <div class="">
                    <h3 class="text-lg font-semibold text-gray-900">Edit Hotel Information</h3>
                    <p class="text-gray-400 font-light">You can update the following information</p>
                </div>
                {{-- @if (Auth::user()->hotelOwner->is_hotel_added) --}}
                    <button type="button" x-on:click="$dispatch('close-edit-hotel-drawer')"
