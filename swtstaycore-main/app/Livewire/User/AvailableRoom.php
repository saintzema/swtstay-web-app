...
class AvailableRoom extends Component
{
    ...
    public function render()
    {
        ...
        if ($currentUser->hotelOwner) {
            $this->rooms =  Room::latest()->whereAvailability(true)->whereHas('branch.hotel', function ($query) use ($currentUser) {
        ...
        if ($currentUser->hotelStaff) {
            $this->rooms =  Room::latest()->whereAvailability(true)->whereHas('branch', function ($query) use ($currentUser) {
                $query->whereHas('hotelStaff', function ($subQuery) use ($currentUser) {
                    $subQuery->where('user_id', $currentUser->id);
                });
            })->get();
        }
        return view('livewire.user.available-room',[
            'rooms' => $this->rooms
        ]);
    }
}
