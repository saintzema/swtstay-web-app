            ...
            <form wire:submit.prevent="finish">
                ...
                <div x-show="step === 1" class="space-y-6">
                    @if (session()->has('status'))
                        <div class="p-3 bg-green-200 border-2 border-green-300 rounded-lg text-sm text-green-500 flex">
                    ...
                    @endif
                    <div>
                        <p class="text-gray-400 text-xs mb-8 select-none">Hotel setup</p>
                        <h2 class="text-2xl font-normal mb-4 select-none">Please provide your hotel information</h2>
                    </div>

                    <div>
                        <label for="name" class="block mb-2 text-sm font-extralight text-gray-900 select-none">What
                            is the name of your hotel?</label>
                        <input type="text" wire:model="firstStepData.name" placeholder="Name of hotel"
                    ...
                    <div class="grid grid-cols-2 space-x-3">
                        <div>
                            <label for="min_price"
                                class="flex mb-2 text-sm font-extralight text-gray-900 select-none items-center">
                                Min price (₦)
                                <button data-popover-target="minPrice" data-popover-placement="right" class="ml-2"
                                    type="button">
                                    <svg class="w-4 text-gray-400 hover:text-gray-500" aria-hidden="true"
                            ...
                            <input type="number" wire:model.blur="firstStepData.minPrice" placeholder="e.g 50,000"
                                class="mt-1 border transition-all ease-in-out border-gray-100 text-gray-500 text-sm font-light hover:shadow-sm shadow-primary-500/10  rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                            @error('firstStepData.minPrice')
                                <span class="text-red-500 text-xs font-extralight">{{ $message }}</span>
                            @enderror
                        </div>
                        <div>
                            <label for="max_price" class="block mb-2 text-sm font-extralight text-gray-900 select-none">
                                Max price (₦)
                                <button data-popover-target="maxPrice" data-popover-placement="right" class="ml-2"
                                    type="button">
                                    <svg class="w-4 text-gray-400 hover:text-gray-500" aria-hidden="true"
                                        fill="currentColor" viewBox="0 0 20 20" xmlns="http://www.w3.org/2000/svg">
                                        <path fill-rule="evenodd"
                            ...
                            <input type="number" wire:model="firstStepData.maxPrice" placeholder="e.g 500,000"
                                class="mt-1 border transition-all ease-in-out border-gray-100 text-gray-500 text-sm font-light hover:shadow-sm shadow-primary-500/10  rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5">
                            @error('firstStepData.maxPrice')
                                <span class="text-red-500 text-xs font-extralight">{{ $message }}</span>
                            @enderror
                        </div>
                    ...
                    <div>
                        <label for="first_name"
                            class="block mb-2 text-sm font-extralight text-gray-900 select-none">Hotel
                            Description</label>
                        <textarea wire:model='firstStepData.description' name="" id="" rows="3"
                        ...
                        @error('firstStepData.description')
                            <span class="text-red-500 text-xs font-extralight">{{ $message }}</span>
                        @enderror
                    </div>

                    {{-- <div>
                        <label for="last_name" class="block mb-2 text-sm font-extralight text-gray-900 select-none">Upload a picture of the hotel (5 max)</label>
                         <x-filepond::upload wire:model="imageFiles" max-files="5" multiple='true' required='true' />
                         <input type="file" name="" id="" wire:model="imageFiles" max="5" multiple required>
                         @error('imageFiles')
                            <span class="text-red-500 text-xs font-extralight">{{ $message }}</span>
                        @enderror
                        @error('imageFiles.*')
                    ...
                    <div x-data="{ uploading: false, progress: 0 }" x-on:livewire-upload-start="uploading = true"
                        ...
                        <div class="border-dashed border-2 border-primary-100 bg-option rounded-lg p-6 text-center cursor-pointer"
                            ...
                            <input type="file" wire:model="imageFiles" x-ref="fileInput" max="5"
                                maxlength="5" class="hidden" multiple>
                        </div>
                ...
                <div x-show="step === 2" class="space-y-6">
                    <div>
                        <p class="text-gray-400 text-xs mb-8 select-none">Final setup</p>
                        <h2 class="text-2xl font-normal mb-4 select-none">Let us know if your hotel has branches</h2>
                    </div>
                    ...
                    <div x-data="{ selected: @entangle('secondStepData.source') }" class="space-y-4">
                        ...
                        <label :class="selected === 'yes' ? 'border-primary-500' : ''"
                            class="flex p-2 border border-gray-300 rounded-lg cursor-pointer justify-between items-center">
                            <span class="text-sm font-light text-gray-700">Yes, we have more than one branch</span>
                            <input type="radio" value="yes"
                                class="rounded border-gray-200 focus:ring-transparent text-primary-500"
                                x-model="selected">
                        </label>

                        <label :class="selected === 'no' ? 'border-primary-500' : ''"
                            class="flex p-2 border border-gray-300 rounded-lg cursor-pointer justify-between items-center">
                            <span class="text-sm font-light text-gray-700">No, we only have one branch for now</span>
